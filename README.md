...
│   ├── Dockerfile                 # Creates the steps necessary for creating the API container.
│   ├── Program.cs                 # The core program that sets up dependencies for the API. 
│   └── Controllers.cs             # The auto-generated controller abstractions generated by Nswag.
├── HomeServer.WebUi/              # Creates a web ui to interact with the TodoList database through the API.
│   ├── Components/                # Contains the razor files that create the web pages.
│   └── .cs             # .
```
